...
class BayesianOptimization(object):
    ...
    def maximize(self, init_points=5, restarts=50, n_iter=25, acq='ei', **gp_params):
        ...
        # only theta0.
        gp = GP(alpha=numpy.random.uniform(0.001, 0.05, self.dim),
                random_state=25)
...
class PrintInfo(object):
    ...
    def print_info(self, op_start, i, x_max, ymax, xtrain, ytrain, keys):
        ...
        if self.lvl:
            ...
            print('Iteration: %3i | Last sampled value: %11f' % ((i + 1), ytrain[-1]), '| with parameters: ',
                  dict(zip(keys, xtrain[-1])))
            print('               | Current maximum: %14f | with parameters: ' % ymax,
                  dict(zip(keys, xtrain[numpy.argmax(ytrain)])))
    ...
    def print_log(self, op_start, i, x_max, xmins, min_max_ratio, ymax, xtrain, ytrain, keys):
        ...
        if self.lvl:
            ...
            numpy.set_printoptions(precision=4, suppress=True)
            print('Iteration: %3i | Last sampled value: %8f' % ((i + 1), ytrain[-1]), '| with parameters: ',
                  dict(zip(keys, return_log(xtrain[-1]))))
            print('               | Current maximum: %11f | with parameters: ' % ymax,
                  dict(zip(keys, return_log(xtrain[numpy.argmax(ytrain)]))))
...
X, Y, Xtarget, Ytarget = train_test_split(train, target, test_size=0.1)
del train
