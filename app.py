...
def upload_files():
   if request.method == 'POST':
      ...
      #tmp = subprocess.call("python compile.py", shell=True)subprocess.call("rm -f ./a.out", shell=True)
      ...
      msg = "Score: " + str(retcode) + " out of 2 correct."

      msg += "\n*************Original submission*************\n"
      with open('uploads/show','r') as fs:
         msg += fs.read()

      return render_template('compile.html',tmp = msg )

  # "<xmp>" + subprocess.call("python compile.py", shell=True) + "</xmp>"
